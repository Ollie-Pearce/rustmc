...
rustup run RustMC cargo test --workspace --target-dir target-ir --no-run --features "inline-more" > "$cargo_output_file" 2>&1
...
cd $TARGET_RUST_PROJECT
find "$(pwd)/target-ir/debug/deps" -type f -name "${PROJECT_NAME}-*.ll" > "$DEPDIR/bitcode.txt"
find "$(pwd)/target-ir/debug/deps" -type f -name "ahash-be0966dcf7255ae3.ll" >> "$DEPDIR/bitcode.txt"
find "$(pwd)/target-ir/debug/deps" -type f -name "hashbrown-c951b5f3b302d426.ll" >> "$DEPDIR/bitcode.txt"
find "$(pwd)/target-ir/debug/deps" -type f -name "allocator_api2-b0f7352188f7450a.ll" >> "$DEPDIR/bitcode.txt"
