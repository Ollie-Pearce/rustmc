...
OVERRIDES_FILE="${OVERRIDES_FILE:-$DEPDIR/overrides.txt}"

cd "$TARGET_RUST_PROJECT"

# Derive stem from $test_file (caller sets test_file)
stem="$(basename "${test_file%.*}")"

# 1) Collect link inputs
tmp_bitcode="$DEPDIR/.bitcode.tmp"
: >"$tmp_bitcode"
...
# b) optionally add project .ll units that do NOT define @main
if [ "$stem" != "$PROJECT_NAME" ]; then
  find "$(pwd)/target-ir/debug/deps" -type f -name "${PROJECT_NAME}*.ll" -print |
...
# c) dedupe
sort -u "$tmp_bitcode" >"$DEPDIR/bitcode.txt"
rm -f "$tmp_bitcode"

# 2) Determine overrides.
#    Example policy: all .ll except the project's own *.ll (space-safe).
: >"$OVERRIDES_FILE"
...
OVERRIDES_FILE="$DEPDIR/overrides.txt"
> "$OVERRIDES_FILE"

cd $TARGET_RUST_PROJECT
find "$(pwd)/target-ir/debug/deps" -type f -name "${PROJECT_NAME}-*.bc" > "$DEPDIR/bitcode.txt"
find "$(pwd)/target-ir/debug/deps" -type f -name '*.ll' ! -name "${PROJECT_NAME}-*.ll" > "$OVERRIDES_FILE"
cmdfile="$(mktemp)"; trap 'rm -f "$cmdfile"' EXIT
{
  printf '%s' "exec /usr/bin/llvm-link-18 --internalize -S --override=$DEPDIR/override/my_pthread.ll"
  while IFS= read -r p; do
    [ -n "$p" ] || continue
    q=$(printf %s "$p" | sed "s/'/'\\\\''/g")   # single-quote escape for POSIX shells
    printf ' %s' "--override='$q'"
  done < "$OVERRIDES_FILE"
  printf ' %s\n' "-o combined_old.ll @bitcode.txt"
} > "$cmdfile"
