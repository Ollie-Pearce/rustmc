...
void spinlock_init(struct spinlock_s *l)
{
...
void spinlock_acquire(struct spinlock_s *l)
{
	do {
		await_for_lock(l);
...
int spinlock_tryacquire(struct spinlock_s *l)
{
	return try_get_lock(l);
}

void spinlock_release(struct spinlock_s *l)
{
