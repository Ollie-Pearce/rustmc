...
class ExperimentRunner:
    ...
    def parse_pipeline_timing(self, traces_dir):
        """Parse pipeline timing from the per-file pipeline_timing.txt"""
        timing_file = traces_dir / 'pipeline_timing.txt'
        return self.parse_timing_file(timing_file)
    ...
    def parse_verification_output(self, output, test_info):
        ...
        # Parse pipeline timing and emit as its own row
        pipeline_timing = self.parse_pipeline_timing(traces_dir)
        results.append({
            'file': rust_file,
            'test_name': 'PIPELINE',
            'should_panic': '',
            'unroll_bound': test_info[0].get('unroll_bound', '1'),
            ...
            'status': 'PIPELINE',
            'matches_expected': '',
            'timing': pipeline_timing if pipeline_timing else {},
        })
    ...
    def generate_csv(self, output_file='experiment_results.csv'):
        ...
        with open(output_file, 'w', newline='') as f:
            ...
            for result in self.results:
                # Get timing info (now per-test, or pipeline timing for PIPELINE rows)
...
def main():
    ...
    # Print summary statistics (exclude PIPELINE rows)
    test_results = [r for r in results if r.get('test_name') != 'PIPELINE']
    print("\n" + "="*80)
    print("SUMMARY")
    print("="*80)
    print(f"Total tests: {len(test_results)}")
    print(f"Matches expected: {sum(1 for r in test_results if r.get('matches_expected'))}")
    print(f"Panics found: {sum(1 for r in test_results if r['panic_found'])}")
