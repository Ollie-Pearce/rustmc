...
# Create rust-toolchain to specify nightly toolchain
echo 'nightly-2024-06-11-x86_64-unknown-linux-gnu' > rust-toolchain
...
RUSTFLAGS="
  ...
  -C passes=memcpyopt \
  -Z mir-opt-level=0 \
  --target=x86_64-unknown-linux-gnu" cargo build
...
find "$(pwd)/target/debug/deps" -name "*.bc" -type f > "$DEPDIR/bitcode.txt"
...
/usr/bin/llvm-link-18 -S --override=$DEPDIR/../override/my_pthread.ll -o combined_old.ll @bitcode.txt
/usr/bin/opt-18 -S -mtriple=x86_64-unknown-linux-gnu -expand-reductions combined_old.ll -o combined.ll

mkdir -p "test_traces_alt_toolchain/${FILENAME}"
mkdir -p "test_results_alt_toolchain"
...
SUMMARY_FILE="test_results_alt_toolchain/${FILENAME}_all_summary.txt"
...
while IFS= read -r ENTRY_FUNCTION; do
    ...
    /usr/bin/time -v -o "test_traces_alt_toolchain/${FILENAME}/${ENTRY_FUNCTION}_timing.txt" \
        timeout 1000s ../genmc --mixer \
            ...
            --unroll="$UNROLL_BOUND" \
            combined.ll > "test_traces_alt_toolchain/${FILENAME}/${ENTRY_FUNCTION}_verification.txt" 2>&1

    GENMC_EXIT=$?

    if [ $GENMC_EXIT -eq 124 ]; then
        echo "TIMEOUT" >> "test_traces_alt_toolchain/${FILENAME}/${ENTRY_FUNCTION}_verification.txt"
    fi

    echo "GenMC exit code: $GENMC_EXIT"
    echo "Last 30 lines of verification output:"
    tail -30 "test_traces_alt_toolchain/${FILENAME}/${ENTRY_FUNCTION}_verification.txt"
    echo " "

    # Analyze results
    cd test_traces_alt_toolchain/${FILENAME}/
...
echo "Individual traces saved to: test_traces_alt_toolchain/${FILENAME}/"
echo "============================================"
