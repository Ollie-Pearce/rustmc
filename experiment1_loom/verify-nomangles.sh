...
/usr/bin/llvm-link-18 --override=$DEPDIR/../override/globals_override.ll -o combined.bc @bitcode.txt

mkdir -p "test_traces/${FILENAME}"
mkdir -p "test_results"

# Check if combined.bc was created
if [ ! -f combined.bc ]; then
    echo "ERROR: combined.bc not found!"
    rm -rf "$TEMP_PROJECT"
    exit 1
fi
...
# Step 1: Run transform pipeline once (timed separately)
echo " "
echo "================= Transforming ${FILENAME} ================="
echo " "

/usr/bin/time -v -o "test_traces/${FILENAME}/pipeline_timing.txt" \
    timeout 1000s ../genmc --mixer \
    --transform-only \
    --transform-output=myout.ll \
    --transform-output-bc="transformed_${FILENAME}.bc" \
    ...
    --disable-estimation \
    --unroll="$UNROLL_BOUND" \
    combined.bc > "test_traces/${FILENAME}/pipeline_output.txt" 2>&1

TRANSFORM_EXIT=$?
if [ $TRANSFORM_EXIT -ne 0 ]; then
    echo "ERROR: Transform pipeline failed with exit code $TRANSFORM_EXIT"
    echo "Transform output:"
    cat "test_traces/${FILENAME}/pipeline_output.txt"
fi

echo "Transform exit code: $TRANSFORM_EXIT"
echo "================= Transform complete ================="
echo ""

# Step 2: Verify ALL tests uniformly with --skip-transforms
while IFS= read -r ENTRY_FUNCTION; do
    ...
    /usr/bin/time -v -o "test_traces/${FILENAME}/${ENTRY_FUNCTION}_timing.txt" \
        ...
        --unroll="$UNROLL_BOUND" \
        "transformed_${FILENAME}.bc" > "test_traces/${FILENAME}/${ENTRY_FUNCTION}_verification.txt" 2>&1
