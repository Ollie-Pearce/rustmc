...
echo "Collecting example tests..."
EXAMPLE_TEST_FILE="$DEPDIR/example_test_functions.txt"
: > "$EXAMPLE_TEST_FILE"


find ./examples -type f -name '*.rs' | while IFS= read -r file; do
  awk '
    BEGIN { in_test = 0 }
    /^[[:space:]]*#\[test\]/ { in_test = 1; next }
    in_test && /^[[:space:]]*fn[[:space:]]+[A-Za-z0-9_]+/ {
      if (match($0, /fn[[:space:]]+([A-Za-z0-9_]+)/, m)) { print m[1] }
      in_test = 0
      next
    }
    /^[[:space:]]*$/ { in_test = 0 }
  ' "$file"
done >> "$EXAMPLE_TEST_FILE"

echo "Example test function names written to: $EXAMPLE_TEST_FILE"
...
/usr/bin/llvm-link-18 --internalize -S --override=$TARGET_RUST_PROJECT/target-ir/debug/deps/sdd-8263b49b5a5542e7.ll -o combined_old.ll
