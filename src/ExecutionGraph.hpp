...
public:
	...
	using co_iterator = ::detail::access_filter_iterator<StoreList::iterator>;
	using const_co_iterator = ::detail::access_filter_iterator<StoreList::const_iterator>;
	using reverse_co_iterator = ::detail::access_filter_iterator<StoreList::reverse_iterator>;
	using const_reverse_co_iterator =
		::detail::access_filter_iterator<StoreList::const_reverse_iterator>;
	...
	co_iterator co_lab_iterator(WriteLabel *lab)
	{
		return co_iterator(StoreList::iterator(lab), co_end(lab->getAccess()),
				     ::detail::OverlapFilter(lab->getAccess()));
	}
	const_co_iterator co_lab_iterator(const WriteLabel *lab) const
	{
		return const_co_iterator(StoreList::const_iterator(lab), co_end(lab->getAccess()),
				     ::detail::OverlapFilter(lab->getAccess()));
	}

	co_iterator co_begin(const AAccess &access)
	{
		auto &locCO = getStoresToLoc(access.getAddr());
		return co_iterator(locCO.begin(), locCO.end(),
				   ::detail::OverlapFilter(access));
	}

	const_co_iterator co_begin(const AAccess &access) const
	{
		auto &locCO = getStoresToLoc(access.getAddr());
		return const_co_iterator(locCO.begin(), locCO.end(),
					 ::detail::OverlapFilter(access));
	};
	co_iterator co_end(const AAccess &access)
	{
		auto &locCO = getStoresToLoc(access.getAddr());
		return co_iterator(locCO.end(), locCO.end(),
				   ::detail::OverlapFilter(access));
	}

	const_co_iterator co_end(const AAccess &access) const
	{
		auto &locCO = getStoresToLoc(access.getAddr());
		return const_co_iterator(locCO.end(), locCO.end(),
					 ::detail::OverlapFilter(access));
	}

	reverse_co_iterator co_rbegin(const AAccess &access)
	{
		auto &locCO = getStoresToLoc(access.getAddr());
		return reverse_co_iterator(locCO.rbegin(), locCO.rend(),
					   ::detail::OverlapFilter(access));
	}
	const_reverse_co_iterator co_rbegin(const AAccess &access) const
	{
		auto &locCO = getStoresToLoc(access.getAddr());
		return const_reverse_co_iterator(locCO.rbegin(), locCO.rend(),
						 ::detail::OverlapFilter(access));
	};
	reverse_co_iterator co_rend(const AAccess &access)
	{
		auto &locCO = getStoresToLoc(access.getAddr());
		return reverse_co_iterator(locCO.rend(), locCO.rend(),
					   ::detail::OverlapFilter(access));
	}
	const_reverse_co_iterator co_rend(const AAccess &access) const
	{
		auto &locCO = getStoresToLoc(access.getAddr());
		return const_reverse_co_iterator(locCO.rend(), locCO.rend(),
						 ::detail::OverlapFilter(access));
	}

	initrf_iterator init_rf_begin(const AAccess &access) { return getInitLabel()->rf_begin(access); }
	const_initrf_iterator init_rf_begin(const AAccess &access) const
	{
		return getInitLabel()->rf_begin(access);
	};
	initrf_iterator init_rf_end(const AAccess &access) { return getInitLabel()->rf_end(access); }
	const_initrf_iterator init_rf_end(const AAccess &access) const { return getInitLabel()->rf_end(access); }

	co_iterator co_succ_begin(WriteLabel *lab, const AAccess &access)
	{
		return ++co_iterator(StoreList::iterator(lab), co_end(access),
				     ::detail::OverlapFilter(access));
	}
	co_iterator co_succ_begin(WriteLabel *lab)
	{
		return co_succ_begin(lab, lab->getAccess());
	}
	const_co_iterator co_succ_begin(const WriteLabel *lab, const AAccess &access) const
	{
		return ++const_co_iterator(StoreList::const_iterator(lab), co_end(access),
					   ::detail::OverlapFilter(access));
	}
	...
	co_iterator co_succ_end(WriteLabel *lab) { return co_end(lab->getAccess()); }
	const_co_iterator co_succ_end(const WriteLabel *lab) const
	{
		return co_end(lab->getAccess());
	}
	...
	{
		return ++reverse_co_iterator(StoreList::reverse_iterator(lab),
					     co_rend(lab->getAccess()),
					     ::detail::OverlapFilter(lab->getAccess()));
	}
	const_reverse_co_iterator co_pred_begin(const WriteLabel *lab) const
	{
		return ++const_reverse_co_iterator(
			StoreList::const_reverse_iterator(lab), co_rend(lab->getAccess()),
			::detail::OverlapFilter(lab->getAccess()));
	}
	reverse_co_iterator co_pred_end(WriteLabel *lab) { return co_rend(lab->getAccess()); }
	const_reverse_co_iterator co_pred_end(const WriteLabel *lab) const
	{
		return co_rend(lab->getAccess());
	}
	...
	co_iterator fr_succ_end(ReadLabel *rLab) { return co_end(rLab->getAccess()); }
	const_co_iterator fr_succ_end(const ReadLabel *rLab) const
	{
		return co_end(rLab->getAccess());
	}
	...
	SAddr getBucket(SAddr addr) const { return addr.align(); }

	const StoreList &getStoresToLoc(SAddr addr) const { return coherence.at(getBucket(addr)); }

	StoreList &getStoresToLoc(SAddr addr) { return coherence[getBucket(addr)]; }
	...
	void addStoresToCO(std::vector<WriteLabel *> &stores, const AAccess &access, StoreList::iterator it)
	{
		auto &locCO = getStoresToLoc(access.getAddr());
		using iterator = llvm::pointee_iterator<std::vector<WriteLabel*>::iterator>;
		locCO.insert(it, iterator(stores.begin()), iterator(stores.end()));
	}
	void addStoreToCO(WriteLabel *wLab, StoreList::iterator it)
	{
		auto &locCO = getStoresToLoc(wLab->getAddr());
		locCO.insert(it, *wLab);
	}
	StoreList::iterator removeStoreFromCO(WriteLabel *wLab)
	{
		auto &locCO = getStoresToLoc(wLab->getAddr());
		return locCO.erase(StoreList::iterator(wLab));
	}
	...
	std::vector<Event> getInitRfsAtLoc(const AAccess &a) const;

	/* Returns true if e is maximal in addr */
	bool isCoMaximal(const AAccess &access, Event e, bool checkCache = false);

	/* Boolean helper functions */

	bool isLocEmpty(const AAccess &access) const { return co_begin(access) == co_end(access); }

	/* Whether a location has more than one store */
	bool hasLocMoreThanOneStore(const AAccess &access) const
	{
		return !isLocEmpty(access) && ++co_begin(access) != co_end(access);
	}
	...
	void trackCoherenceAtLoc(const AAccess &access);

	/* Graph modification methods */

	void changeRf(Event read, Event store);
	void changeRf(Event read, ReadRevisitMIXER::RfsT rfs);
