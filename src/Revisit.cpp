...
llvm::raw_ostream &operator<<(llvm::raw_ostream &s, const Revisit::Kind k)
{
	...
	case Revisit::RV_FRevRead:
		s << "FR";
		break;
	case Revisit::RV_FRevRead_MIXER:
		s << "FR_MIXER";
		break;
...
llvm::raw_ostream &operator<<(llvm::raw_ostream &s, const Revisit &item)
{
	...
	case Revisit::RV_FRevRead_MIXER: {
		auto &fi = static_cast<const ReadForwardRevisitMIXER &>(item);
		s << fi.getKind() << "(" << fi.getPos() << ": " << format(fi.getRfs()) << ")";
		break;
	}
	case Revisit::RV_FRevMO: {
	...
	case Revisit::RV_FRevMO_MIXER: {
		auto &mi = static_cast<const WriteRevisitMIXER &>(item);
		s << mi.getKind() << "(" << mi.getPos() << ": " << mi.getSucc() << ")"
			<< " with seq " << format(mi.getSeq());
		break;
	}
	case Revisit::RV_FRevOpt: {
