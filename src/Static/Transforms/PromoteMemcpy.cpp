		...
		if (auto *memset_length_const = dyn_cast<ConstantInt>(MI->getLength())) {
			...
			for (uint64_t i = 0; i < memset_length_const->getZExtValue(); i++) {
				...
				Value *addr = builder.CreateInBoundsGEP(
					builder.getInt8Ty(), dst, offset, "memset.gep");
				...
				SmallVector<llvm::MemIntrinsic *> &promoted)
{
	...
	/* Copy gap/padding bytes one at a time to avoid multi-byte loads
	 * that can span coherence bucket boundaries in MIXER mode. */
	for (int i = 0; i < uninitialisedFields.size(); i++) {
		for (uint64_t j = uninitialisedFields[i].first;
		     j < uninitialisedFields[i].second; ++j) {
			Value *index = ConstantInt::get(builder.getInt64Ty(), j);
			Value *srcGEP = builder.CreateGEP(int8Type,
							  MemCpyToConvert->getSource(),
							  index, "gap_src_gep");
			Value *loadedVal = builder.CreateLoad(int8Type, srcGEP,
							      "gap_load");
...
auto promoteMemcpy(MemCpyInst *MI, SmallVector<llvm::MemIntrinsic *> &promoted) -> bool
{
	...
	/* Copy byte-by-byte to avoid generating multi-byte loads that can
	 * span coherence bucket boundaries (8-byte aligned words) in MIXER.
	 * A load i64 from a non-8-byte-aligned address can straddle two
	 * buckets, causing MIXER to miss overlapping writes in the adjacent
	 * bucket and report false uninitialized-memory errors. */
...
auto PromoteUndefMemset(MemSetInst *MI, SmallVector<llvm::MemIntrinsic *> &promoted)
{
	...
	/* Use stores no wider than the destination alignment to avoid
	 * crossing 8-byte coherence bucket boundaries in MIXER. */
	uint64_t align = 1;
	if (auto *AI = dyn_cast<AllocaInst>(MemsetDest->stripPointerCasts()))
		align = AI->getAlign().value();
	else if (auto destAlign = MI->getDestAlign())
		align = destAlign->value();
	uint64_t stepSize = std::min(align, uint64_t(8));
	Type *StoreTy = IntegerType::get(MI->getContext(), stepSize * 8);
	Value *UndefVal = UndefValue::get(StoreTy);

	Value *Len = MI->getLength();
	if (auto *CI = dyn_cast<ConstantInt>(Len)) {
		uint64_t lengthVal = CI->getZExtValue();

		IRBuilder<> builder(MI);

		for (uint64_t i = 0; i < lengthVal; i += stepSize) {
			Value *Indices[1] = {
				ConstantInt::get(Type::getInt64Ty(MI->getContext()), i)};
			Value *ElemPtr = builder.CreateGEP(I8Ty, MemsetDest,
							   Indices, "elemPtr");
