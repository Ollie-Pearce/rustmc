...
#include "Transforms/FixRustAllocLTOPass.hpp"
...
#include "Transforms/FoldInterpreterUnsupportedConstants.hpp"
			 ...
			 const std::shared_ptr<const Config> &conf) -> bool
{
	...
	/*
	 * Phase 1: Run passes up to and including IntrinsicLoweringPass.
	 * IntrinsicLoweringPass converts llvm.memcpy intrinsics to call @memcpy(...)
	 */
	llvm::ModulePassManager preLinkMGR;

	preLinkMGR.addPass(DeclareInternalsPass());
	preLinkMGR.addPass(FixRustAllocLTOPass());
	preLinkMGR.addPass(DefineLibcFunsPass());
	preLinkMGR.addPass(MDataCollectionPass(PI));

	if (conf->inlineFunctions)
		preLinkMGR.addPass(FunctionInlinerPass());

	{
		llvm::FunctionPassManager fpm;

		/* Expand vector reduction intrinsics (e.g. llvm.vector.reduce.add)
		 * into scalar operations before scalarizing remaining vectors. */
		fpm.addPass(llvm::ExpandReductionsPass());

		/* Scalarize vector operations from fat LTO's SLP vectorizer.
		 * GenMC's interpreter does not support vector types. */
		{
			ScalarizerPassOptions opts;
			opts.ScalarizeLoadStore = true;
			fpm.addPass(ScalarizerPass(opts));
		}

		fpm.addPass(FoldInterpreterUnsupportedConstants());

		fpm.addPass(PromoteMemcpy());

		/* PromoteMemIntrinsicPass handles constant-length memcpy by converting
		 * them to explicit load/store sequences (field-by-field for structs).
		 * Remaining memcpy intrinsics (dynamic length or failed promotion) will
		 * be converted to calls by IntrinsicLoweringPass below. */
		...
		/* IntrinsicLoweringPass converts remaining llvm.memcpy/memmove/memset
		 * intrinsics to function calls @memcpy/@memmove/@memset. */
		fpm.addPass(IntrinsicLoweringPass());

		preLinkMGR.addPass(llvm::createModuleToFunctionPassAdaptor(std::move(fpm)));
	}

	/* Run Phase 1 passes */
	preLinkMGR.run(mod, mam);

	/* Verify module after preLinkMGR passes */
	if (llvm::verifyModule(mod, &llvm::errs())) {
		ERROR("Module verification failed after preLinkMGR passes!\n");
	}

	/*
	 * Phase 2: Continue with remaining passes.
	 * memcpy/memmove/memset remain as declarations (no body) —
	 * they are handled at runtime by ExternalFunctions.cpp handlers.
	 */
...
void writeBitcodeModule(llvm::Module &mod, const std::string &out)
{
	std::error_code errs;
	auto os = std::make_unique<llvm::raw_fd_ostream>(out.c_str(), errs,
							 llvm::sys::fs::OF_None);
	if (errs) {
		WARN("Failed to write bitcode module to file " + out + ": " + errs.message());
		return;
	}
	llvm::WriteBitcodeToFile(mod, *os);
}

void collectModuleInfo(llvm::Module &mod, ModuleInfo &MI)
{
	PassModuleInfo PI;
	...
	mgr.addPass(MDataCollectionPass(PI));
	mgr.run(mod, mam);
