...
Event ExecutionGraph::getPendingRMW(const WriteLabel *sLab) const
{
	...
	std::ranges::for_each(pLab->getRfs(), [&](const auto &p){
		auto [rfLab, access] = p;
		/* Fastpath: non-init rf */
		auto readFromOverlaps = [&](const ReadLabel *rLab, const EventLabel *sLab) {
			return std::ranges::any_of(rLab->getRfs(), [&](const auto &rf){
				return rf.first == sLab && access.overlaps(rf.second);
			});
		};
		if (auto *wLab = llvm::dyn_cast<WriteLabel>(rfLab)) {
			std::for_each(wLab->readers_begin(), wLab->readers_end(), [&](auto &rLab) {
				if (isRMWLoad(&rLab) && &rLab != pLab && readFromOverlaps(&rLab, wLab))
					pending.push_back(rLab.getPos());
			});
		} else {
			/*  Slowpath: scan init rfs */
			std::for_each(init_rf_begin(access), init_rf_end(access), [&](auto &rLab) {
				if (isRMWLoad(&rLab) && &rLab != pLab && readFromOverlaps(&rLab, getInitLabel()))
					pending.push_back(rLab.getPos());
			});
		}
	});
	...
	for (auto i = 0u; i < getNumThreads(); i++) {
		for (auto j = before.getMax(i) + 1u; j < getThreadSize(i); j++) {
			...
			if (auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
				if (rLab->overlaps(sLab) && rLab->isRevisitable() &&
				    rLab->wasAddedMax())
...
/* Returns a vector with all reads that read from INIT and overlap with A */
std::vector<Event> ExecutionGraph::getInitRfsAtLoc(const AAccess &a) const
{
	std::vector<Event> result;

	for (const auto &lab : labels(*this)) {
		if (auto *rLab = llvm::dyn_cast<ReadLabel>(&lab))
			if (std::ranges::any_of(rLab->getRfs(), [&a](const auto &p){
				return p.first->getPos().isInitializer() && p.second.overlaps(a);
			}))
...
bool ExecutionGraph::isCoMaximal(const AAccess &access, Event e, bool checkCache /* = false */)
{
	if (e.isInitializer())
		return isLocEmpty(access);
	return !isLocEmpty(access) && e == co_rbegin(access)->getPos();
}

void ExecutionGraph::trackCoherenceAtLoc(const AAccess &access)
{
	getStoresToLoc(access.getAddr()); // just for side-effects
	getInitLabel()->initRfs[access.getAddr().align()];
}
...
void ExecutionGraph::removeLast(unsigned int thread)
{
	...
	if (auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
		std::ranges::for_each(rLab->getRfs(), [&](const auto &p){
			if (auto *wLab = llvm::dyn_cast_or_null<WriteLabel>(p.first)) {
...
{
	// TODO: Why not get readers and check?
	for (const auto &lab : labels(*this)) {
		if (!isRMWLoad(&lab))
			continue;
		...
		if (std::ranges::any_of(rLab->getRfs(), [&](const auto &p){
			return p.first->getPos() == store && rLab->getAddr() == ptr;
		}))
			return true;
	}
	return false;
					     ...
					     const VectorClock &prefix) const
{
	...
	for (const auto &lab : labels(*this)) {
		...
		auto *rLab = llvm::dyn_cast<ReadLabel>(&lab);
		if (std::ranges::none_of(rLab->getRfs(), [&](const auto &p){
			return p.first->getPos() == store && rLab->getAddr() == ptr;
		}))
			continue;
...
void ExecutionGraph::changeRf(Event read, ReadRevisitMIXER::RfsT rfs)
{
	/* First, we set the new reads-from edge */
	auto *rLab = llvm::dyn_cast<ReadLabel>(getEventLabel(read));
	BUG_ON(!rLab);
	auto oldRfLabs = rLab->getRfs();
	ReadLabel::RfsT rfsLab;
	std::transform(rfs.begin(), rfs.end(), std::back_inserter(rfsLab), [&](auto p){
		return std::make_pair(getEventLabel(p.first), p.second);
	});
	rLab->setRf(rfsLab);
	 ...
	 *        during a revisit)
	 *     3) That oldRf is not the initializer */
	std::ranges::for_each(oldRfLabs, [&](const auto &p){
		auto oldRfLab = p.first;
		if (oldRfLab && containsPos(oldRfLab->getPos())) {
			BUG_ON(!containsLab(oldRfLab));
			if (auto *oldLab = llvm::dyn_cast<WriteLabel>(oldRfLab))
				oldLab->removeReader([&](ReadLabel &oLab) { return &oLab == rLab; });
	...
	/* Add it to the write's reader list */
	std::vector<Event> stores;
	std::ranges::for_each(rfs, [&](const auto &p){
		if (std::ranges::find(stores, p.first) == stores.end())
			stores.push_back(p.first);
	});
	std::ranges::for_each(stores, [&](Event store){
		if (auto *wLab = llvm::dyn_cast<WriteLabel>(getEventLabel(store))) {
			wLab->addReader(rLab);
		} else {
			BUG_ON(!store.isInitializer());
			addInitRfToLoc(rLab);
...
void ExecutionGraph::removeAfter(const VectorClock &preds)
{
	...
	for (auto i = 0u; i < preds.size(); i++) {
		for (auto j = 0u; j <= preds.getMax(i); j++) {
			auto *lab = getEventLabel(Event(i, j));
			if (auto *mLab = llvm::dyn_cast<MemAccessLabel>(lab))
				keep.insert(getBucket(mLab->getAddr()));
		}
	}
	...
	for (auto lIt = coherence.begin(); lIt != coherence.end(); /* empty */) {
		...
		if (!keep.count(getBucket(lIt->first))) {
...
void ExecutionGraph::copyGraphUpTo(ExecutionGraph &other, const VectorClock &v) const
{
	...
	for (auto i = 0u; i < getNumThreads(); i++) {
		...
		for (auto j = 1; j <= v.getMax(i); j++) {
			...
			// See GenMCDriver::backwardRevisit
			if (!containsPos(Event(i, j)))
				continue;
			...
			if (auto *mLab = llvm::dyn_cast<MemAccessLabel>(nLab))
				other.trackCoherenceAtLoc(mLab->getAccess());
	...
	for (auto &lab : labels(other)) {
		auto *rLab = llvm::dyn_cast<ReadLabel>(&lab);
		if (rLab) {
			const auto rfs = rLab->getRfs();
			rLab->clearRfs();
			std::ranges::for_each(rfs, [&](const auto &p){
				if (other.containsPos(p.first->getPos()))
					rLab->addRf(other.getEventLabel(p.first->getPos()), p.second);
			});
		}
		if (auto *wLab = llvm::dyn_cast<WriteLabel>(&lab)) {
	...
	for (auto lIt = loc_begin(), lE = loc_end(); lIt != lE; ++lIt) {
		for (auto sIt = lIt->second.begin(); sIt != lIt->second.end(); ++sIt)
			if (v.contains(sIt->getPos())) {
				other.addStoreToCO(other.getWriteLabel(sIt->getPos()), other.co_end(sIt->getAccess()));
			}
	}
...
void ExecutionGraph::validate(void)
{
	for (auto &lab : labels(*this)) {
		if (auto *rLab = llvm::dyn_cast<ReadLabel>(&lab)) {
			...
			std::ranges::for_each(rLab->getRfs(), [&](const auto &p){
				if (auto *rfLab = llvm::dyn_cast<WriteLabel>(p.first)) {
		...
		if (auto *wLab = llvm::dyn_cast<WriteLabel>(&lab)) {
			...
			if (std::any_of(wLab->readers_begin(), wLab->readers_end(),
					[&](auto &rLab) {
					return std::ranges::none_of(rLab.getRfs(), [wLab](const auto &p){
						return p.first == wLab;
					});
			})) {
