...
void Interpreter::collectStaticAddresses(SAddrAllocator &alloctor)
{
	 ...
	 * For non-TLS globals this fixes up pointer values within initializers
	 * (they may reference other globals that have been remapped to SAddr).
	 * For TLS globals this is the *first* initialization — LLVM's
	 * EmitGlobalVariable skips InitializeMemory for thread-local globals,
	 * so the EE memory still contains uninitialized malloc garbage. */
	for (auto &p : toReinitialize)
		InitializeMemory(p.first->getInitializer(), p.second);

	/* Populate threadLocalVars from the now-correctly-initialized EE memory.
	 * We use SAddr keys (via getConstantValue) since the globals have
	 * already been remapped by updateGlobalMapping above.
	 *
	 * Hybrid strategy: for scalar types (int, ptr, float, double) we use
	 * getConstantValue on the initializer to produce a properly-typed
	 * GenericValue (correct IntVal width, PointerVal, FloatVal, etc.).
	 * For aggregate types (struct, array) we fall back to byte-by-byte
	 * decomposition since getConstantValue crashes on aggregates. */
	for (auto &p : toReinitialize) {
		if (!p.first->isThreadLocal())
			continue;
		Type *valTy = p.first->getValueType();
		char *saddrBase = reinterpret_cast<char *>(
			GVTOP(getConstantValue(p.first)));
		if (valTy->isIntegerTy() || valTy->isPointerTy() ||
		    valTy->isFloatTy() || valTy->isDoubleTy()) {
			/* Scalar: getConstantValue produces a properly-typed GenericValue */
			GenericValue gv = getConstantValue(p.first->getInitializer());
			threadLocalVars[saddrBase] = gv;
		} else {
			/* Aggregate fallback: store byte-by-byte from EE memory */
			char *eeBase = static_cast<char *>(p.second);
			unsigned typeSize = getDataLayout().getTypeAllocSize(valTy);
			for (unsigned j = 0; j < typeSize; ++j) {
				GenericValue ByteGV;
				ByteGV.IntVal = APInt(8, static_cast<uint8_t>(eeBase[j]));
				threadLocalVars[saddrBase + j] = ByteGV;
			}
		}
	}
}
