...
auto isEnabled(const ExecutionGraph &g, const View &v, int t) -> bool
{
	...
	if (auto *lLab = llvm::dyn_cast<LockCasReadLabel>(nLab)) {
		auto access = lLab->getAccess();
		// Get latest store in the view
		auto it = std::find_if(g.co_rbegin(access), g.co_rend(access),
				       [&](const auto &w) { return v.contains(w.getPos()); });

		// No such store exists
		if (it == g.co_rend(access)) {
			return std::none_of(g.getInitLabel()->rf_begin(access),
					    g.getInitLabel()->rf_end(access),
	...
	if (auto *bLab = llvm::dyn_cast<BWaitReadLabel>(nLab)) {
		auto access = bLab->getAccess();
		// Get barrrier initialization value
		auto *iwLab = llvm::dyn_cast<WriteLabel>(&*g.co_begin(access));
		BUG_ON(!iwLab);
		auto it = std::find_if(g.co_rbegin(access), g.co_rend(access),
				       [&](const auto &w) { return v.contains(w.getPos()); });
...
auto isSCMaximal(const ExecutionGraph &g, const View &v, int t) -> bool
{
	...
	auto access = llvm::cast<MemAccessLabel>(lab)->getAccess();
	if (g.isLocEmpty(access))
		return true;

	auto *succLab = llvm::isa<InitLabel>(wLab) ? &*g.co_begin(access)
