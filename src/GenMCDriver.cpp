...
{
	auto last = (store_rbegin(g, lab->getAccess()) == store_rend(g, lab->getAccess()))
			    ? Event::getInit()
			    : store_rbegin(g, lab->getAccess())->getPos();
...
void GenMCDriver::checkHelpingCasAnnotation()
{
	...
	for (auto &h : hs) {
		...
		if (std::any_of(store_begin(g, hLab->getAccess()), store_end(g, hLab->getAccess()),
				[&](auto &sLab) {
		...
		if (hLab->getAddr().isStatic() &&
			...
			auto rs = g.collectAllEvents([&](const EventLabel *lab) {
				...
				// FIXME: Correct MIXER behavior?
				BUG_ON(getConf()->mixer);
				return rLab && rLab->getAccess() == hLab->getAccess();
			});
...
SVal GenMCDriver::getWriteValue(const EventLabel *lab, const AAccess &access)
{
	 ...
	 * sizes should match */
	if (!getConf()->mixer && (wLab->getSize() != access.getSize()))
	...
	return wLab->getSizedValAtOffset(access);
}

SVal GenMCDriver::getWriteValue(const ReadLabel::RfsT &rfs, const AAccess &access)
{
	SVal ret;
	std::ranges::for_each(rfs, [&](const auto &p){
		auto offset = (p.second.getAddr() - access.getAddr())*8;
		auto val = getWriteValue(p.first, p.second);
		ret |= (val << offset);
	});
	return ret;
}
...
SVal GenMCDriver::getDskWriteValue(const ReadLabel::RfsT &rfs)
{
	SVal ret;
	std::ranges::for_each(rfs, [&](const auto &p){
		ret ^= p.first->getPos().isInitializer() ?
			SVal() : getWriteValue(p.first, p.second);
	});
	return ret;
}
...
{
	const auto &g = getGraph();
	auto sIt = std::find_if(store_begin(g, access), store_end(g, access),
				[&access, &g](auto &bLab) {
...
bool GenMCDriver::isCoMaximal(const AAccess &access, Event e, bool checkCache /* = false */)
{
	return getGraph().isCoMaximal(access, e, checkCache);
}

bool GenMCDriver::isCoMaximal(std::vector<std::pair<Event, AAccess>> writes, bool checkCache /* = false */)
{
	return std::ranges::all_of(writes, [&](const auto &p){
		return getGraph().isCoMaximal(p.second, p.first, checkCache);
	});
}
...
VerificationError GenMCDriver::checkInitializedMem(const ReadLabel *rLab)
{
	...
	if (lLab && getWriteValue(lLab->getRfs(), lLab->getAccess()) == SVal(-1)) {
	...
	if (bLab && std::ranges::any_of(bLab->getRfs(), [](const auto &p)
				{ return p.first->getPos().isInitializer(); })) {
		reportError({rLab->getPos(), VerificationError::VE_UninitializedMem,
			     "Called barrier_wait() on uninitialized barrier!"});
		return VerificationError::VE_UninitializedMem;
	}
	if (bLab && getWriteValue(bLab->getRfs(), bLab->getAccess()) == SVal(0)) {
	...
	if (std::ranges::any_of(rLab->getRfs(), [](const auto &p) {
			return isUninitializedAccess(p.second.getAddr(), p.first->getPos());
		})) {
		reportError({rLab->getPos(), VerificationError::VE_UninitializedMem});
		return VerificationError::VE_UninitializedMem;
	}
...
VerificationError GenMCDriver::checkInitializedMem(const WriteLabel *wLab)
{
	...
	if (bLab && std::any_of(store_begin(g, bLab->getAccess()), store_end(g, bLab->getAccess()),
				[&](auto &sLab) {
...
void GenMCDriver::checkFinalAnnotations(const WriteLabel *wLab)
{
	...
	if (g.hasLocMoreThanOneStore(wLab->getAccess()))
		return;
	if ((wLab->isFinal() &&
	     std::any_of(store_begin(g, wLab->getAccess()), store_end(g, wLab->getAccess()),
...
void GenMCDriver::checkIPRValidity(const ReadLabel *rLab)
{
	...
	auto &g = getGraph();
	auto racyIt = std::find_if(store_begin(g, rLab->getAccess()), store_end(g, rLab->getAccess()),
				   [&](auto &wLab) { return wLab.hasAttr(WriteAttr::WWRacy); });
	if (racyIt != store_end(g, rLab->getAccess())) {
...
bool GenMCDriver::threadReadsMaximal(int tid)
{
	...
	for (auto j = start.index; j > 0; j--) {
		...
		if (auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
			if (std::ranges::any_of(rLab->getRfs(), [&](const auto &p){
				return !isCoMaximal(p.second, p.first->getPos());
			}))
				return false;
		}
	}
...
bool GenMCDriver::isEcoBefore(const EventLabel *lab, int tid) const
{
	...
	if (auto *sLab = llvm::dyn_cast<WriteLabel>(lab))
		if (std::any_of(g.co_succ_begin(sLab), g.co_succ_end(sLab), [&](auto &sLab) {
...
bool GenMCDriver::isEcoSymmetric(const EventLabel *lab, int tid) const
{
	...
	if (auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
		return rLab->getRfs() == llvm::dyn_cast<ReadLabel>(symmLab)->getRfs();
	}
...
void GenMCDriver::updatePrefixWithSymmetriesSR(EventLabel *lab)
{
	...
	if (auto *rLab = llvm::dyn_cast<ReadLabel>(symmLab)) {
		std::ranges::for_each(rLab->getRfs(), [&](const auto &p){
			v.update(getPrefixView(p.first));
		});
	}
}
...
int GenMCDriver::calcLargestSymmPrefixBeforeSR(int tid, Event pos) const
{
	...
	for (auto j = 0; j < limit; j++) {
		...
		if (auto *rLabA = llvm::dyn_cast<ReadLabel>(labA)) {
			...
			if ((std::ranges::all_of(rLabA->getRfs(), [tid](const auto &p){ return p.first->getThread() == tid; }))
			 && (std::ranges::all_of(rLabB->getRfs(), [pos](const auto &p){ return p.first->getThread() == pos.thread; }))
			 && (std::ranges::equal(rLabA->getRfs(), rLabB->getRfs(), [](const auto &p1, const auto &p2){ return p1.first->getIndex() == p2.first->getIndex(); }))
			 )
				continue;
			if (rLabA->getRfs() != rLabB->getRfs())
				return j - 1;
		}
...
void GenMCDriver::filterSymmetricStoresSR(const ReadLabel *rLab, std::vector<Event> &stores) const
{
	...
	// TODO: MIXER requires that stores becomes vector<vector<Event>> (?)
	BUG_ON(getConf()->mixer);
	stores.erase(std::remove_if(stores.begin(), stores.end(),
	...
	validStores.erase(std::remove_if(validStores.begin(), validStores.end(),
					 [&](Event w) {
						 auto val = getWriteValue(g.getEventLabel(w),
									  rLab->getAccess());
						 return w != maximal &&
							!isCoMaximal(rLab->getAccess(), w, true) &&
...
{
	auto rfs = getCoherentStores(lab);
	if (!llvm::isa<CasReadLabel>(lab) && !llvm::isa<FaiReadLabel>(lab))
		return rfs;

	// TODO: Under MIXER, you can read from (part of) a write that is read by a settled RMW
	if (getConf()->mixer)
		return rfs;
	...
	stores.erase(std::remove_if(
			     ...
			     [&](auto &s) {
				     ...
				     if (auto *iLab = llvm::dyn_cast<InitLabel>(sLab))
					     return std::any_of(
						     iLab->rf_begin(rLab->getAccess()),
						     iLab->rf_end(rLab->getAccess()),
						     [&](auto &rLab) {
...
std::optional<SVal> GenMCDriver::pickRandomRf(ReadLabel *rLab, std::vector<Event> &stores)
{
	...
	if (checkInitializedMem(rLab) != VerificationError::VE_OK)
		return std::nullopt;
	return getWriteValue(rLab->getRfs(), rLab->getAccess());
}

void GenMCDriver::enumerateRfHelper(const SAddr word, std::vector<ReadRevisitMIXER::RfsT> &options,
		     std::vector<Event> stores, const ReadRevisitMIXER::RfsT current,
		     const AAccess::WordFragment remaining, const ReadLabel *rLab,
		     std::set<Event, EventL2RCompare> mustUse)
{
	/* Current choice covers the whole fragment */
	if (remaining.empty()) {
		/* All mustUse writes were used */
		if (mustUse.empty())
			options.push_back(current);
		return;
	}

	/* We run out of stores, but there is still remaining fragment */
	if (stores.empty())
		return;
	...
	stores.pop_back();
	auto *wLab = g.getWriteLabel(store);
	auto overlap = wLab ? wLab->getAccess().getOverlap(remaining) : remaining;
	/*  Either *first* read from the whole (due to coherence) write */
	if (!overlap.empty()) {
		auto newCurrent(current);
		auto newMustUse(mustUse);
		/* Overlapping bytes read from the store */
		if (std::ranges::all_of(AAccess::getAccesses(word, overlap), [&](auto a){
			auto [sIt, eIt] = wLab ?
				std::make_pair(g.co_succ_begin(wLab, a), g.co_succ_end(wLab)) :
				std::make_pair(g.co_begin(a), g.co_end(a));
			/* Filter out ra-inconsistent choices: fr;rf? to the read's hb-prefix */
			if (std::any_of(sIt, eIt, [&](const WriteLabel &succ){
				/* If an event is (co|a;rf)|a-after rLab, it is also co|a;rf|a-after rLab.
				 * A weird example is rLab acessing xy, wLab is W y 1,
				 * succ is W (y 2, z 1), and a reader of W being R (y 2, z != 1).
				 * Due to coherence, the write with value != 1 is co-after W (y 2, z 1),
				 * and therefore co|a;rf|a-after wLab */
				return isWriteRfBefore(succ.getPos(), rLab->getPos(), a);
			}))
				return false;
			newCurrent.push_back({store, a});
			return true;
		})) {
			newMustUse.erase(store);
			enumerateRfHelper(word, options, stores, newCurrent, remaining.subtract(overlap), rLab, newMustUse);
		}
	}
	/* Or do not read at all */
	enumerateRfHelper(word, options, stores, current, remaining, rLab, mustUse);
}

std::vector<ReadRevisitMIXER::RfsT> GenMCDriver::enumerateRfOptions(std::vector<Event> stores, const AAccess &access,
		const ReadLabel *rLab, std::set<Event, EventL2RCompare> mustUse)
{
	auto &g = getGraph();
	std::vector<ReadRevisitMIXER::RfsT> options;
	AAccess::WordFragment fragment(access);
	enumerateRfHelper(access.getAddr().align(), options, stores, {}, fragment, rLab, mustUse);
	/* Coherence-maximal is at the end */
...
std::optional<SVal> GenMCDriver::handleLoad(std::unique_ptr<ReadLabel> rLab)
{
	...
	g.trackCoherenceAtLoc(rLab->getAccess());
	...
	// FIXME: MIXER
	if (!getConf()->mixer && !filterOptimizeRfs(lab, stores))
		return std::nullopt;

	auto rfOptions = enumerateRfOptions(stores, lab->getAccess(), lab);
	if (rfOptions.empty()) {
		const auto &access = lab->getAccess();
		auto startByte = access.getAddr().getByte();
		auto endByte = startByte + (int)access.getSize().get() - 1;
		bool crossesWord = endByte >= 8;
		std::string str;
		llvm::raw_string_ostream out(str);
		out << "No valid reads-from options for load " << lab->getPos()
		    << " at " << access << " (word byte range: " << startByte
		    << "-" << endByte << ").\n";
		if (crossesWord) {
			out << "This access crosses an 8-byte word boundary "
			    << "(starts at byte " << startByte << " of its word, "
			    << "spans " << access.getSize() << " bytes). "
			    << "GenMC's coherence tracker cannot handle cross-word accesses.\n";
		}
		WARN(out.str());
		BUG();
	}
	/* ... add an appropriate label with a random rf */
	g.changeRf(lab->getPos(), rfOptions.back());
	 ...
	 */
	// FIXME: MIXER
	if (!getConf()->mixer && !ensureConsistentRf(lab, stores))
		return std::nullopt;
	...
	std::for_each(rfOptions.begin(), rfOptions.end() - 1, [&](const auto &rfs) {
		addToWorklist(lab->getStamp(),
			      std::make_unique<ReadForwardRevisitMIXER>(lab->getPos(), rfs));
	});
	return {retVal};
}
...
std::pair<std::vector<Event>, int> constructCoSeq(const ExecutionGraph &g, const A<Event> &set,
		size_t lsize, const WriteLabel *sLab)
{
	...
	auto overlaps = 0;
	if (!lsize)
		seq.push_back(sLab->getPos());
	for (auto e : set) {
		seq.push_back(e);
		if (sLab->overlaps(g.getWriteLabel(e)) && (++overlaps == lsize)) {
			index = seq.size();
			seq.push_back(sLab->getPos());
		}
	}
...
template<template<class> class A>
std::vector<std::vector<Event>> getPerByteCoherence(const ExecutionGraph &g,
		const A<Event> &coh, const AAccess &access)
{
	std::vector<std::vector<Event>> result;
	int bytes;
	SAddr::Width bucket = access.getAddr().align().get();
	for (int i = 0; i < 8; i++) {
		AAccess byteAccess(SAddr(bucket + i), ASize(1), AType::Signed);
		std::vector<Event> byteResult;
		for (const auto &e : coh) {
			if (g.getWriteLabel(e)->overlaps(byteAccess))
				byteResult.push_back(e);
		}
		result.push_back(std::move(byteResult));
	}
	return result;
}
...
void GenMCDriver::calcCoOrderings(WriteLabel *lab)
{
	 ...
	 */
	auto &g = getGraph();
	auto placesRange = getCoherentPlacings(lab);
  ...
  if (std::all_of(placesRange.begin(), placesRange.end(), [&](const WriteLabel &wLab) {
        return wLab.getAccess().getAddr() == lab->getAccess().getAddr() &&
               wLab.getAccess().getSize() == lab->getAccess().getSize();
  })) {
    /* We cannot place the write just before the write of an RMW or during recovery */
    for (auto &succLab : placesRange) {
      if (!g.isRMWStore(succLab.getPos()) &&
          !isWriteRfBefore(succLab.getPos(), lab->getPos(), lab->getAccess())
      )
	...
	const auto &bucket = g.getStoresToLoc(lab->getAddr());
	// FIXME: Why doesn't for_each_permutation work with std::vector?
	llvm::SmallVector<Event> coh;
	for (const auto &lab : g.getStoresToLoc(lab->getAddr()))
		coh.push_back(lab.getPos());
	const auto order = getPerByteCoherence(g, coh, lab->getAccess());
	struct cmpOrders {
		bool operator()(const std::vector<std::vector<Event>> &lhs,
				const std::vector<std::vector<Event>> &rhs) const {
			return std::lexicographical_compare(lhs.begin(), lhs.end(),
							    rhs.begin(), rhs.end(),
			    [](const std::vector<Event> &lhs, const std::vector<Event> &rhs){
			    return std::lexicographical_compare(lhs.begin(), lhs.end(),
					    rhs.begin(), rhs.end(), EventL2RCompare());
			    }
			);
		}
	};
	std::map<
		...
		cmpOrders
		> cache;
	const auto overlapping = std::distance(g.co_begin(lab->getAccess()), g.co_end(lab->getAccess()));
	for_each_permutation(coh.begin(), coh.end(), coh.end(),
		[&](Event *first, Event *last) {
		/* Does the permutation introduce any new orderings? */
		if (getPerByteCoherence(g, coh, lab->getAccess()) != order)
			return false;
		/* Place the new write before each overlapping write */
		for (auto i = 0; i < overlapping; i++) {
			const auto &[newCoh, index] = constructCoSeq(g, coh, i, lab);
			BUG_ON(newCoh[index] != lab->getPos());
			auto remaining = AAccess::WordFragment(lab->getAccess());
			/* Iterate over co-successors in the new coherence */
			if (std::any_of(newCoh.begin() + index + 1, newCoh.end(), [&](auto pos){
				auto *nLab = g.getWriteLabel(pos);
				auto overlap = nLab->getAccess().getOverlap(remaining);
				/* We cannot place the write just before the write of an RMW or during recovery */
				if (!overlap.empty() && g.isRMWStore(pos))
					return true;
				remaining = remaining.subtract(overlap);
				/* Filter out ra-inconsistent placements */
				if (lab->overlaps(nLab) && isWriteRfBefore(nLab->getPos(), lab->getPos(), lab->getAccess()))
					return true;
				return false;
			}))
				continue;
			cache[getPerByteCoherence(g, newCoh, lab->getAccess())] = newCoh;
		}
		return false;
	});

	for (auto [_order, val]: cache)
		addToWorklist(lab->getStamp(), std::make_unique<WriteRevisitMIXER>(
				lab->getPos(), lab->getPos() /* dummy */, std::move(val)));
...
void GenMCDriver::handleStore(std::unique_ptr<WriteLabel> wLab)
{
	...
	g.trackCoherenceAtLoc(wLab->getAccess());
	...
	for (auto pos = start.prev(); pos.index > 0; --pos) {
		...
		if (auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
			if (getConf()->helper && g.isConfirming(rLab))
				continue;
			if (rLab->hasRf()) {
				if (std::ranges::all_of(rLab->getRfs(), [](const auto &p){
					auto *wLab = llvm::dyn_cast<WriteLabel>(p.first);
					return wLab && wLab->isLocal();
				}))
					continue;
				if (std::ranges::all_of(rLab->getRfs(), [](const auto &p){
					auto *wLab = llvm::dyn_cast<WriteLabel>(p.first);
					return wLab && wLab->isFinal();
				})) {
					     ...
					     std::vector<Event> &loads)
{
	...
	std::for_each(bs.begin(), bs.end(), [&](const Event &b) {
		...
		rLab->setAddedMax(std::ranges::all_of(rLab->getRfs(), [&](const auto &p) {
			return isCoMaximal(p.second, p.first->getPos());
		}));
	});
	return true;
...
void GenMCDriver::optimizeUnconfirmedRevisits(const WriteLabel *sLab, std::vector<Event> &loads)
{
	...
	auto valid = std::count_if(
		store_begin(g, sLab->getAccess()), store_end(g, sLab->getAccess()), [&](auto &wLab) {
...
bool GenMCDriver::tryOptimizeRevBlockerAddition(const WriteLabel *sLab, std::vector<Event> &loads)
{
	...
	if (std::find_if(store_begin(g, sLab->getAccess()), store_end(g, sLab->getAccess()),
			 [this, pLab, sLab](auto &lab) {
				 return isConflictingNonRevBlocker(pLab, sLab, lab.getPos());
			 }) != store_end(g, sLab->getAccess())) {
		moot();
		loads.clear();
		return true;
	}
		...
		for (auto j = 1; j <= pporf.getMax(i); j++) {
			...
			if (auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
				...
				if (preds.contains(rLab->getPos())) {
					std::ranges::for_each(rLab->getRfs(), [&](const auto &p){
						auto rfLab = p.first;
						if (!preds.contains(rfLab) && rfLab->getThread() == rLab->getThread())
							predsD.removeHole(rfLab->getPos());
					});
				}
			}
...
GenMCDriver::getRevisitView(const ReadLabel *rLab, const WriteLabel *sLab,
			    const WriteLabel *midLab /* = nullptr */,
			    std::vector<Event> keepSet /* = {} */) const
{
	auto &g = getGraph();
	auto preds = g.getPredsView(rLab->getPos());

	updatePredsWithPrefixView(g, *preds, getPrefixView(sLab));
	std::ranges::for_each(keepSet, [&](auto pos) {
		updatePredsWithPrefixView(g, *preds, getPrefixView(g.getEventLabel(pos)));
	});
	if (midLab)
...
std::unique_ptr<BackwardRevisit> GenMCDriver::constructBackwardRevisit(const ReadLabel *rLab,
				       ...
				       ReadRevisitMIXER::RfsT rfs /* = {} */,
				       std::vector<Event> keepSet /* = {} */) const
{
	if (getConf()->mixer)
		return std::make_unique<BackwardRevisitMIXER>(rLab, sLab, getRevisitView(rLab, sLab, nullptr, keepSet), rfs);
...
bool GenMCDriver::hasBeenRevisitedByDeleted(const BackwardRevisit &r, const EventLabel *eLab)
{
	...
	return std::ranges::any_of(lab->getRfs(), [&](const auto &p){
		return !v.contains(p.first->getPos()) && p.first->getStamp() > lab->getStamp() &&
		!prefixContainsSameLoc(r, p.first);
	});
}
...
bool GenMCDriver::isCoBeforeSavedPrefix(const BackwardRevisit &r, const EventLabel *lab)
{
	...
	std::vector<std::pair<const EventLabel*, const AAccess>> ws;
	if (auto *rLab = llvm::dyn_cast<ReadLabel>(mLab))
		std::transform(rLab->getRfs().begin(), rLab->getRfs().end(), std::back_inserter(ws),
				[](const auto &p){ return std::make_pair(p.first, p.second); });
	else
		ws.push_back(std::make_pair(mLab, mLab->getAccess()));

	return std::ranges::any_of(ws, [&](const auto &p){
		auto w = llvm::dyn_cast_or_null<WriteLabel>(p.first);
		auto access = p.second;
		auto succIt = w ? g.co_succ_begin(w, access) : g.co_begin(access);
		auto succE = g.co_end(access);
...
bool GenMCDriver::isMaximalExtension(const BackwardRevisit &r)
{
	...
	/* MIXER: Read should not read from deleted events */
	if (auto *mr = llvm::dyn_cast<BackwardRevisitMIXER>(&r))
		if (std::ranges::any_of(mr->getRfs(), [&](const auto &p){
			return !r.getViewNoRel()->contains(p.first);
		}))
			return false;
...
void enumerateSubsetHelper(const ExecutionGraph &g, std::vector<std::vector<Event>> &keepSets,
		std::vector<Event> stores, std::vector<Event> current, const VectorClock &view)
{
	/* No more choices, explored one subset */
	if (stores.empty()) {
		/* Each keep set should be co-sorted */
		std::ranges::reverse(current);
		keepSets.push_back(current);
		return;
	}
	 auto store = stores.back();
	 stores.pop_back();

	 /* Explore not keeping this store */
	 enumerateSubsetHelper(g, keepSets, stores, current, view);

	 /* Explore keeping the store, if is it not alredy covered */
	 if (!view.contains(store)) {
		 auto newView = view.clone();
		 newView->setMax(store);
		 current.push_back(store);
		 enumerateSubsetHelper(g, keepSets, stores, current, *newView);
	 }
}
void GenMCDriver::enumerateKeepCandidates(std::vector<std::vector<Event>> &keepSets, Event read, Event store) const
{
	auto &g = getGraph();

	auto preds = g.getPredsView(read);
	updatePredsWithPrefixView(g, *preds, getPrefixView(g.getWriteLabel(store)));

	std::vector<Event> stores;
	EventL2RCompare l2r;
	auto *rLab = g.getReadLabel(read);
	std::for_each(g.co_begin(rLab->getAccess()), g.co_end(rLab->getAccess()), [&](const WriteLabel &wLab) {
		auto keepCandidate = wLab.getPos();
		/* We can only keep stores that were added after the read
		 * and they are porf-independent of the write */
		if (preds->contains(keepCandidate))
			return;
		BUG_ON(l2r(keepCandidate, read));
		/* We can only keep stores to the left of the revisiting store */
		if (l2r(keepCandidate, store))
			stores.push_back(keepCandidate);
	});
	enumerateSubsetHelper(g, keepSets, stores, {}, *preds);
}
...
bool GenMCDriver::calcRevisits(const WriteLabel *sLab)
{
	...
	EventL2RCompare l2r;
	for (auto &l : loads) {
		std::vector<std::vector<Event>> keepSetCandidates;
		auto *rLab = g.getReadLabel(l);
		if (getConf()->mixer) {
			BUG_ON(getConf()->schedulePolicy != SchedulePolicy::ltr);
			if (!l2r(l, sLab->getPos()))
				continue;
      if (std::all_of(g.co_begin(rLab->getAccess()),
            g.co_end(rLab->getAccess()),
            [&](const WriteLabel &wLab) {
              auto access = wLab.getAccess();
              auto rAccess = rLab->getAccess();
              return access.getAddr() == rAccess.getAddr() && access.getSize() == rAccess.getSize();
      }))
        keepSetCandidates.push_back({});
      else {
        enumerateKeepCandidates(keepSetCandidates, l, sLab->getPos());
      }
		}
		BUG_ON(!rLab);

		auto stores = getRfsApproximation(rLab);
		BUG_ON(std::ranges::find(stores, sLab->getPos()) == stores.end());
		for (auto keepSet : keepSetCandidates) {
			std::set<Event, EventL2RCompare> mustUse(keepSet.begin(), keepSet.end());
			mustUse.insert(sLab->getPos());
			for (auto rfs : enumerateRfOptions(stores , rLab->getAccess(), rLab, mustUse)) {
				auto br = constructBackwardRevisit(rLab, sLab, rfs, keepSet);
				if (isMaximalExtension(*br))
					addToWorklist(sLab->getStamp(), std::move(br));
				// FIXME: Break out of both loops, as before?
			}
		}
	}
...
#define CREATE_COUNTERPART(name)                                                                   \
	...
	std::vector<WriteLabel *> before;
	std::vector<WriteLabel *> after;
	auto remaining = rLab->getRfs();
	std::vector<AAccess> seen;

	/* If any remaining rf reads from sLab, mark it as seen and remove it */
	auto markSeen = [&](auto &sLab){
		for (auto it = remaining.begin(); it != remaining.end(); /* empty */) {
			if (it->first == &sLab) {
	...
	/* Mark as seen any rfs from Init */
	markSeen(*g.getInitLabel());

	auto &coh = g.getStoresToLoc(lab->getAddr());
	std::ranges::for_each(coh, [&](auto &sLab){
		/* If the write is co-after a seen access, put it after the RMW write */
		if (std::ranges::any_of(seen, [&](const auto &a){ return a.overlaps(sLab.getAccess()); }))
			after.push_back(&sLab);
		else {
			/* Otherwise, (possibly) mark (part of) the access and put it before the RWM write */
			markSeen(sLab);
			before.push_back(&sLab);
		}
	});

	/* New coherence bucket is [before ++ RMW write ++ after] */
	coh.clear();
	g.addStoresToCO(before, lab->getAccess(), coh.end());
	g.addStoreToCO(lab, coh.end());
	g.addStoresToCO(after, lab->getAccess(), coh.end());
...
bool GenMCDriver::revisitWrite(const WriteForwardRevisit &ri)
{
	...
	if (auto *mm = llvm::dyn_cast<WriteRevisitMIXER>(&ri)) {
		std::vector<WriteLabel*> seq;
		std::transform(mm->getSeq().begin(), mm->getSeq().end(),
		 std::back_inserter(seq), [&](const auto &e){ return g.getWriteLabel(e); });
		auto &coh = g.getStoresToLoc(wLab->getAddr());
		coh.clear();
		g.addStoresToCO(seq, wLab->getAccess(), coh.end());
		BUG_ON(coh.size() != seq.size());

	} else
		g.addStoreToCO(wLab, ExecutionGraph::StoreList::iterator(
					g.getWriteLabel(ri.getSucc())));
...
bool GenMCDriver::revisitRead(const Revisit &ri)
{
	...
	rLab->setAddedMax(fri ? fri->isMaximal() : isCoMaximal(rLab->getAccess(), rev));
...
bool GenMCDriver::revisitReadMIXER(const Revisit &rr, const ReadRevisitMIXER &rm)
{

	/* We are dealing with a read: change its reads-from and also check
	 * whether a part of an RMW should be added */
	auto &g = getGraph();
	auto *rLab = llvm::dyn_cast<ReadLabel>(g.getEventLabel(rr.getPos()));
	auto rfs = rm.getRfs();

	g.changeRf(rLab->getPos(), rfs);
	auto *fri = llvm::dyn_cast<ReadForwardRevisit>(&rr);
	rLab->setAddedMax(fri ? fri->isMaximal() : isCoMaximal(rfs));
	rLab->setIPRStatus(false);

	GENMC_DEBUG(LOG(VerbosityLevel::Debug1)
			    << "--- " << (llvm::isa<BackwardRevisit>(rr) ? "Backward" : "Forward")
			    << " revisiting " << rr.getPos() << " <-- " << format(rfs) << " (MIXER)\n"
			    << getGraph(););
...
bool GenMCDriver::forwardRevisit(const ForwardRevisit &fr)
{
	...
	BUG_ON(!ri);
	if (auto *rm = llvm::dyn_cast<ReadForwardRevisitMIXER>(&fr))
		return revisitReadMIXER(*ri, *rm);
	BUG();
...
bool GenMCDriver::backwardRevisit(const BackwardRevisit &br)
{
	...
	auto *brm = llvm::dyn_cast<BackwardRevisitMIXER>(&br);
	/* TODO: Recalculate also in MIXER case ? */
	auto v = brm ? brm->getViewNoRel()->clone() : getRevisitView(g.getReadLabel(br.getPos()), g.getWriteLabel(br.getRev()),
	...
	auto ok = brm ? revisitReadMIXER(br, *brm) : revisitRead(br);
	/* MIXER doesn't guarantee that  you can add the possible following write */
	BUG_ON(!ok && !getConf()->mixer);
...
SVal GenMCDriver::handleDskRead(std::unique_ptr<DskReadLabel> drLab)
{
	...
	g.trackCoherenceAtLoc(drLab->getAccess());
...
void GenMCDriver::handleDskWrite(std::unique_ptr<DskWriteLabel> wLab)
{
	...
	auto placesRange = getCoherentPlacings(&*wLab);
			     ...
			     llvm::raw_ostream &s /* = llvm::dbgs() */)
{
	...
	for (auto locIt = g.loc_begin(), locE = g.loc_end(); locIt != locE; ++locIt) {
		...
		auto access = static_cast<AAccess>(locIt->first);
		if (g.hasLocMoreThanOneStore(access)) {
			if (!header) {
				s << "Coherence:\n";
				header = true;
			}
			auto *wLab = &*g.co_begin(access);
			s << getVarName(wLab->getAddr()) << ": [ ";
			for (const auto &w : stores(g, access))
				s << w << " ";
			s << "]\n";
		}
	}
		...
		for (auto j = 0; j <= before->getMax(i); j++) {
			...
			if (j < before->getMax(i) && !llvm::isa<ThreadStartLabel>(lab))
				ss << "\"" << lab->getPos() << "\" -> \"" << lab->getPos().next()
				   << "\"\n";
			if (auto *rLab = llvm::dyn_cast<ReadLabel>(lab)) {
				ss << "\"" << format(rLab->getRfs()) << "\" -> \"" << rLab->getPos()
