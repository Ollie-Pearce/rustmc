...
/* Stores a value into a thread's TLS map, decomposing integers into
 * byte-level entries to stay consistent with the byte-by-byte
 * initialization format used in collectStaticAddresses(). */
static void storeTLSValue(std::unordered_map<const void *, GenericValue> &tls,
			  void *ptr, const GenericValue &val, Type *typ)
{
	if (typ->isIntegerTy() && typ->getIntegerBitWidth() >= 8) {
		unsigned bytes = typ->getIntegerBitWidth() / 8;
		char *base = reinterpret_cast<char *>(ptr);
		for (unsigned j = 0; j < bytes; ++j) {
			GenericValue byteVal;
			byteVal.IntVal = APInt(8, val.IntVal.extractBitsAsZExtValue(8, j * 8));
			tls[base + j] = byteVal;
		}
	} else {
		tls[ptr] = val;
...
void Interpreter::visitLoadInst(LoadInst &I)
{
	...
	if (thr.tls.count(ptr)) {
		GenericValue val = thr.tls[ptr];
		/* TLS vars are initialized byte-by-byte (APInt(8,...) per byte).
		 * If this is an integer load wider than the stored entry,
		 * reconstruct the full-width value from the byte entries. */
		if (typ->isIntegerTy() && val.IntVal.getBitWidth() < typ->getIntegerBitWidth()) {
			unsigned expected = typ->getIntegerBitWidth();
			char *base = reinterpret_cast<char *>(ptr);
			APInt fullVal(expected, 0);
			for (unsigned j = 0; j < expected / 8; ++j) {
				auto it = thr.tls.find(base + j);
				if (it != thr.tls.end())
					fullVal |= APInt(expected, it->second.IntVal.getZExtValue()) << (j * 8);
			}
			val.IntVal = fullVal;
		} else if (typ->isPointerTy() && val.IntVal.getBitWidth() == 8) {
			/* Pointer loaded from byte-decomposed aggregate TLS entry:
			 * reconstruct the pointer from individual byte entries. */
			unsigned bytes = getDataLayout().getPointerSize();
			char *base = reinterpret_cast<char *>(ptr);
			uint64_t ptrVal = 0;
			for (unsigned j = 0; j < bytes; ++j) {
				auto it = thr.tls.find(base + j);
				if (it != thr.tls.end())
					ptrVal |= (uint64_t)it->second.IntVal.getZExtValue() << (j * 8);
			}
			val.PointerVal = reinterpret_cast<void *>(ptrVal);
		}
		SetValue(&I, val, ECStack().back());
		return;
	}
...
void Interpreter::visitStoreInst(StoreInst &I)
{
	...
	if (thr.tls.count(ptr)) {
		storeTLSValue(thr.tls, ptr, val, typ);
		return;
	}
...
void Interpreter::visitAtomicCmpXchgInst(AtomicCmpXchgInst &I)
{
	...
	if (thr.tls.count(ptr)) {
		...
		/* Reconstruct full-width integer from byte entries (see visitLoadInst) */
		if (typ->isIntegerTy() && oldVal.IntVal.getBitWidth() < typ->getIntegerBitWidth()) {
			unsigned expected = typ->getIntegerBitWidth();
			char *base = reinterpret_cast<char *>(ptr);
			APInt fullVal(expected, 0);
			for (unsigned j = 0; j < expected / 8; ++j) {
			...
			oldVal.IntVal = fullVal;
		} else if (typ->isPointerTy() && oldVal.IntVal.getBitWidth() == 8) {
			/* Pointer from byte-decomposed aggregate TLS (see visitLoadInst) */
			unsigned bytes = getDataLayout().getPointerSize();
			char *base = reinterpret_cast<char *>(ptr);
			uint64_t ptrVal = 0;
			for (unsigned j = 0; j < bytes; ++j) {
			...
			oldVal.PointerVal = reinterpret_cast<void *>(ptrVal);
		}
		GenericValue cmpRes = executeICMP_EQ(oldVal, cmpVal, typ);
		if (cmpRes.IntVal.getBoolValue())
			storeTLSValue(thr.tls, ptr, newVal, typ);
...
void Interpreter::visitAtomicRMWInst(AtomicRMWInst &I)
{
	...
	if (thr.tls.count(ptr)) {
		...
		auto newVal = executeAtomicRMWOperation(GV_TO_SVAL(oldVal, typ), val, size,
							I.getOperation());
		storeTLSValue(thr.tls, ptr, SVAL_TO_GV(newVal, typ), typ);
...
/*
 * Memory intrinsic handlers.
 * These are intercepted here (rather than as external functions) so that
 * each byte-level load/store creates a proper event in the execution graph,
 * allowing the model checker to track accesses and detect data races.
 *
 * Constant-size intrinsics are already promoted to explicit load/store
 * sequences by PromoteMemcpy/PromoteMemIntrinsicPass/PromoteMemMove.
 * Only the remaining (dynamic-length) cases reach these handlers.
 */

void Interpreter::callMemcpy(Function *F, const std::vector<GenericValue> &ArgVals,
			     const std::unique_ptr<EventDeps> &specialDeps)
{
	auto *dest = (char *)GVTOP(ArgVals[0]);
	auto *src = (char *)GVTOP(ArgVals[1]);
	auto len = (size_t)ArgVals[2].IntVal.getZExtValue();

	for (size_t i = 0; i < len; i++) {
		auto val = CALL_DRIVER(handleLoad,
				       ReadLabel::create(currPos(), AtomicOrdering::NotAtomic,
							 src + i, ASize(1), AType::Unsigned,
							 GET_DEPS(specialDeps)));
		if (!val.has_value())
			return;

		CALL_DRIVER(handleStore,
			    WriteLabel::create(currPos(), AtomicOrdering::NotAtomic, dest + i,
					       ASize(1), AType::Unsigned, val.value(),
					       GET_DEPS(specialDeps)));
	}

	GenericValue result;
	result.PointerVal = dest;
	returnValueToCaller(F->getReturnType(), result);
}

void Interpreter::callMemmove(Function *F, const std::vector<GenericValue> &ArgVals,
			      const std::unique_ptr<EventDeps> &specialDeps)
{
	auto *dest = (char *)GVTOP(ArgVals[0]);
	auto *src = (char *)GVTOP(ArgVals[1]);
	...
	/* Then write all bytes */
	for (size_t i = 0; i < len; i++) {
		CALL_DRIVER(handleStore,
			    WriteLabel::create(currPos(), AtomicOrdering::NotAtomic, dest + i,
					       ASize(1), AType::Unsigned, bytes[i],
					       GET_DEPS(specialDeps)));
	}
...
void Interpreter::callMemset(Function *F, const std::vector<GenericValue> &ArgVals,
			     const std::unique_ptr<EventDeps> &specialDeps)
{
	auto *dest = (char *)GVTOP(ArgVals[0]);
	auto val = SVal(ArgVals[1].IntVal.getZExtValue() & 0xFF);
	auto len = (size_t)ArgVals[2].IntVal.getZExtValue();

	for (size_t i = 0; i < len; i++) {
		CALL_DRIVER(handleStore,
			    WriteLabel::create(currPos(), AtomicOrdering::NotAtomic, dest + i,
					       ASize(1), AType::Unsigned, val,
					       GET_DEPS(specialDeps)));
	}
				       ...
				       const std::unique_ptr<EventDeps> &specialDeps)
{
	...
	switch (fCode) {
		...
		CALL_INTERNAL_FUNCTION(Memcpy);
		CALL_INTERNAL_FUNCTION(Memmove);
		CALL_INTERNAL_FUNCTION(Memset);
	default:
		BUG();
		break;
	}
...
void Interpreter::callFunction(Function *F, const std::vector<GenericValue> &ArgVals,
			       const std::unique_ptr<EventDeps> &specialDeps)
{
	/* Detect rust_begin_unwind (panic) and halt with trace */
	if (F->getName() == "rust_begin_unwind") {
		driver->reportError({currPos(), VerificationError::VE_Safety,
		                     "Thread panicked (rust_begin_unwind called)"});
		return;
	}
