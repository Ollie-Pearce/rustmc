...
public:
	...
	/* Helper function */
	void enumerateRfHelper(const SAddr word, std::vector<ReadRevisitMIXER::RfsT> &options,
	...
	/* Enumerate all possible ways for an access read from the (co-sorted) set of stores */
	std::vector<ReadRevisitMIXER::RfsT> enumerateRfOptions(std::vector<Event> stores, const AAccess &access,
...
protected:
	...
	/* Lift getWriteValue to vector of (lab, access) */
	SVal getWriteValue(const ReadLabel::RfsT &rfs, const AAccess &a);
	...
	/* Lift getDskWriteValue to vector of (lab, access) */
	SVal getDskWriteValue(const ReadLabel::RfsT &rfs);
	...
	{
		return getDskWriteValue(rLab->getRfs());
	}
	...
	/* Returns true if E is maximal in ACCESS's address at P*/
	bool isCoMaximal(const AAccess &access, Event e, bool checkCache = false);

	bool isCoMaximal(std::vector<std::pair<Event, AAccess>> writes, bool checkCache = false);
...
private:
	...
	std::unique_ptr<BackwardRevisit> constructBackwardRevisit(const ReadLabel *rLab,
					  const WriteLabel *sLab, ReadRevisitMIXER::RfsT rfs = {},
	...
	std::unique_ptr<VectorClock> getRevisitView(const ReadLabel *rLab, const WriteLabel *sLab,
				    const WriteLabel *midLab = nullptr, std::vector<Event> keepSet = {}) const;
	...
	void enumerateKeepCandidates(std::vector<std::vector<Event>> &keepSets, Event read, Event store) const;
	...
	bool revisitReadMIXER(const Revisit &s, const ReadRevisitMIXER &m);
	...
	virtual std::vector<Event> getCoherentStores(const ReadLabel *rLab) = 0;
	virtual llvm::iterator_range<ExecutionGraph::co_iterator>
	getCoherentPlacings(const WriteLabel *wLab) = 0;

	virtual bool isWriteRfBefore(Event a, Event b, const AAccess &access) const = 0;
