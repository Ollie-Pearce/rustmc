...
public:
	...
	{
		return (r->getKind() >= Revisit::RV_FRevRead && r->getKind() <= Revisit::RV_FRevReadLast) ||
...
public:
	...
	// TODO: MIXER, do it properly
	ReadForwardRevisit(Event r, bool m = false)
		: ForwardRevisit(RV_FRevRead_MIXER, r), maximal(m), ReadRevisit(RV_FRevRead_MIXER, {})
	{}
	...
	static bool classof(const Revisit *item)
	{
		return item->getKind() >= RV_FRevRead && item->getKind() <= RV_FRevReadLast;
	}
...
class ReadRevisitMIXER {

public:
	using RfsT = std::vector<std::pair<Event, AAccess>>;
	ReadRevisitMIXER(RfsT rfs) : rfs(rfs) {}

	const RfsT &getRfs() const { return rfs; }
...
/*
 * ReadForwardRevisitMIXER class - Forward revisit a read by multiple writes
 */

class ReadForwardRevisitMIXER : public ReadForwardRevisit, public ReadRevisitMIXER {

public:
	ReadForwardRevisitMIXER(Event r, ReadRevisitMIXER::RfsT rfs) : ReadForwardRevisit(r), ReadRevisitMIXER(rfs) {}

	static bool classof(const Revisit *item) {
		return item->getKind() == RV_FRevRead_MIXER;
	}
};
...
public:
	...
	static bool classof(const Revisit *item) {
		return (item->getKind() >= RV_FRevMO) && (item->getKind() <= RV_FRevMOLast);
	}
...
/*
 * WriteRevisitMIXER class - Represents an alternative MO position for a store
 * in a mixed-size accesses setting. We need to explore all possible combinations
 * of co-neighbors that "commute"
 */
class WriteRevisitMIXER : public WriteForwardRevisit {

public:
	WriteRevisitMIXER(Event p, Event succ, std::vector<Event> &&seq)
		: WriteForwardRevisit(RV_FRevMO_MIXER, p, succ), seq(seq) {}

	const std::vector<Event> &getSeq() const { return seq; }

	static bool classof(const Revisit *item) {
		return item->getKind() == RV_FRevMO_MIXER;
	}
...
/*
 * BackwardRevisitMIXER class - Represents a MIXER backward revisit
 */
class BackwardRevisitMIXER : public BackwardRevisit, public ReadRevisitMIXER {

public:
	BackwardRevisitMIXER(const ReadLabel *rLab, const WriteLabel *wLab,
			      std::unique_ptr<VectorClock> view, ReadRevisitMIXER::RfsT rfs = {})
		: ReadRevisitMIXER(rfs), BackwardRevisit(RV_BRevMIXER, rLab->getPos(), wLab->getPos(), std::move(view))
	{}

	static bool classof(const Revisit *item) { return item->getKind() == RV_BRevMIXER; }
	static ReadRevisit *castToReadRevisit(const BackwardRevisitMIXER *r)
	{
		return static_cast<ReadRevisit *>(const_cast<BackwardRevisitMIXER *>(r));
	}
	static BackwardRevisitMIXER *castFromReadRevisit(const ReadRevisit *r)
	{
		return static_cast<BackwardRevisitMIXER *>(const_cast<ReadRevisit *>(r));
	}
};
...
inline Revisit *Revisit::castFromReadRevisit(const ReadRevisit *r)
{
	...
	case Revisit::Kind::RV_BRevHelper:
		return static_cast<BackwardRevisitHELPER *>(const_cast<ReadRevisit *>(r));
	case Revisit::Kind::RV_BRevMIXER:
		return static_cast<BackwardRevisitMIXER *>(const_cast<ReadRevisit *>(r));
	default:
		BUG();
	}
...
inline ReadRevisit *Revisit::castToReadRevisit(const Revisit *r)
{
	...
	case Revisit::Kind::RV_BRevMIXER:
		return static_cast<BackwardRevisitMIXER *>(const_cast<Revisit *>(r));
	default:
		BUG();
	}
}
