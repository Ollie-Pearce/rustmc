...
public:
	...
	/* Returns whether this access overlaps with another access */
	bool overlaps(const MemAccessLabel *lab) const {
		return getAccess().overlaps(lab->getAccess());
	}
	bool overlaps(const AAccess &access) const {
		return getAccess().overlaps(access);
	}
...
public:
	...
	using RfsT = std::vector<std::pair<EventLabel *, AAccess>>;
...
public:
	...
	EventLabel *getRf() const {
		if (hasRf())
			return rfs[0].first;
		return nullptr;
	}

	const RfsT &getRfs() const {
		return rfs;
	}
	...
	bool readsExt() const
	{
		return hasRf() && !getRf()->getPos().isInitializer() &&
...
private:
	...
	void addRf(EventLabel *lab, const AAccess access) {
		rfs.emplace_back(lab, access);
	}

	/* Removes all rfs that originate from pos */
	void removeRf(Event pos) {
		auto [first, last] = std::ranges::remove_if(rfs, [pos](const auto &p){
			return p.first->getPos() == pos;
		});
		rfs.erase(first, last);
	}
...
public:
	...
	/* Mixer: Returns the part of the value specified by A as a separate value.
	 * Pre: A's address is contained in the write's access */
	SVal getSizedValAtOffset(const AAccess &a) const {
		BUG_ON(!getAccess().contains(a.getAddr()));
		return (getVal() >> (SVal(a.getAddr() - getAddr())*SVal(8))) &
			(SVal(~0) >> (64 - a.getSize().get()*8)) ;
	}
	...
	using ReaderList = std::vector<ReadLabel*>; //CopyableIList<ReadLabel>;
	using rf_iterator = llvm::pointee_iterator<ReaderList::iterator>;
	using const_rf_iterator = llvm::pointee_iterator<ReaderList::const_iterator>;
	using rf_range = llvm::iterator_range<rf_iterator>;
	using const_rf_range = llvm::iterator_range<const_rf_iterator>;

	/* TODO: MIXER: It should? also track which part each readers is reading from
	 * Check that users do not depend on the assumption that every part of the access
...
private:
	...
	{
		std::erase_if(readerList, [&cond](ReadLabel *rLab){ return cond(*rLab); });
	}
...
public:
	...
	using rf_iterator = ::detail::access_filter_iterator<llvm::pointee_iterator<ReaderList::iterator>>;
	using const_rf_iterator = ::detail::access_filter_iterator<llvm::pointee_iterator<ReaderList::const_iterator>>;

	/* FIXME: Returns iterator to events that read from INIT and overlap with access,
	 * but not necessarily reading (part of) access from INIT. */
	rf_iterator rf_begin(const AAccess &access)
	{
		auto &bucket = initRfs[access.getAddr().align()];
		return rf_iterator(bucket.begin(), bucket.end(), ::detail::OverlapFilter(access));
	}
	const_rf_iterator rf_begin(const AAccess &access) const
	{
		auto &bucket = initRfs.at(access.getAddr().align());
		return const_rf_iterator(bucket.begin(), bucket.end(), ::detail::OverlapFilter(access));
	}

	rf_iterator rf_end(const AAccess &access)
	{
		auto &bucket = initRfs[access.getAddr().align()];
		return rf_iterator(bucket.end(), bucket.end(), ::detail::OverlapFilter(access));
	}
	const_rf_iterator rf_end(const AAccess &access) const
	{
		auto &bucket = initRfs.at(access.getAddr().align());
		return const_rf_iterator(bucket.end(), bucket.end(), ::detail::OverlapFilter(access));
	}
...
private:
	void addReader(ReadLabel *rLab)
	{
		BUG_ON(std::find_if(rf_begin(rLab->getAccess()), rf_end(rLab->getAccess()),
				    [rLab](ReadLabel &oLab) {
					    return oLab.getPos() == rLab->getPos();
				    }) != rf_end(rLab->getAccess()));
		initRfs[rLab->getAddr().align()].push_back(rLab);
	}

	/* Removes all readers that satisfy predicate F */
	template <typename F> void removeReader(SAddr addr, F cond)
	{
		std::erase_if(initRfs[addr.align()], [&cond](ReadLabel *rLab){ return cond(*rLab); });
	}

	/* Map from bucket (aligned address) to readers */
