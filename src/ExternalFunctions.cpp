...
{
	/*
	 * Memory intrinsics should _not_ be executed here, as they need to be
	 * intercepted by the interpreter to create proper model checking events.
	 * They are handled as internal functions in Execution.cpp.
	 */
...
{
	/* See lle_X_memset comment above */
...
static GenericValue lle_X_memcmp(FunctionType *FT, const std::vector<GenericValue> &Args)
{
	/*
	 * memcmp takes pointer arguments that are virtual addresses in the
	 * interpreter's address space. Calling the native memcmp via FFI
	 * would dereference these invalid pointers and segfault.
	 */
	ERROR("ERROR: Unsupported external function: memcmp(). "
	      "The interpreter cannot forward virtual addresses to native memcmp. "
	      "Consider providing an LLVM IR implementation.\n");
...
static GenericValue lle_X_clock_gettime(FunctionType *FT, const std::vector<GenericValue> &Args)
{
	/*
	 * clock_gettime is now handled via LLVM IR in my_pthread.ll.
	 * This fallback should not be reached.
	 */
	ERROR("ERROR: clock_gettime() reached external function handler. "
	      "It should be provided via the LLVM IR override (my_pthread.ll).\n");
...
void Interpreter::initializeExternalFunctions()
{
	...
	FuncNames["lle_X_memcmp"] = lle_X_memcmp;
	FuncNames["lle_X_clock_gettime"] = lle_X_clock_gettime;
}
