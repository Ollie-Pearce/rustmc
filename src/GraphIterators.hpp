...
inline const_store_iterator store_begin(const ExecutionGraph &G, const AAccess &access)
{
	return G.co_begin(access);
}
inline const_store_iterator store_end(const ExecutionGraph &G, const AAccess &access)
{
	return G.co_end(access);
}
inline const_store_range stores(const ExecutionGraph &G, const AAccess &access)
{
	return const_store_range(store_begin(G, access), store_end(G, access));
}

inline const_reverse_store_iterator store_rbegin(const ExecutionGraph &G, const AAccess &access)
{
	return G.co_rbegin(access);
}
inline const_reverse_store_iterator store_rend(const ExecutionGraph &G, const AAccess &access)
{
	return G.co_rend(access);
}
inline const_reverse_store_range rstores(const ExecutionGraph &G, const AAccess &access)
{
	return const_reverse_store_range(store_rbegin(G, access), store_rend(G, access));
}
...
namespace detail {
// FIXME: Very ugly
inline llvm::simple_ilist<WriteLabel> emptyList;
inline AAccess dummyAccess(SAddr(), ASize(0), AType::Unsigned);
inline const_co_iterator coSentinel(emptyList.begin(), emptyList.end(), ::detail::OverlapFilter(dummyAccess));
inline const_reverse_co_iterator coRevSentinel(emptyList.rbegin(), emptyList.rend(), ::detail::OverlapFilter(dummyAccess));
}; // namespace detail
...
//	return lab && lab->hasRf() ? const_reverse_detour_iterator(
...
//{
//	auto *lab = G.getReadLabel(e);
//	auto pos = lab && lab->hasRf() ? lab->getRf()->getPos() : Event::getInit();
...
inline const EventLabel *rf_pred(const ExecutionGraph &G, const EventLabel *lab)
{
	...
	return (!rLab || !rLab->hasRf()) ? nullptr : rLab->getRf();
}
