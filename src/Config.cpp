...
static llvm::cl::opt<bool> clMixer("mixer", llvm::cl::cat(clGeneral),
				    llvm::cl::desc("Enable MIXER"));
...
static llvm::cl::list<std::string> clProgramEntryFunction(
	"program-entry-function", llvm::cl::value_desc("fun_name"),
	llvm::cl::cat(clDebugging),
...
static llvm::cl::opt<bool>
	clSkipTransforms("skip-transforms", llvm::cl::cat(clDebugging),
			 llvm::cl::desc("Skip the LLVM transformation pipeline (use with pre-transformed bitcode)"));

static llvm::cl::opt<bool>
	clTransformOnly("transform-only", llvm::cl::cat(clDebugging),
			llvm::cl::desc("Only run the transformation pipeline, then exit (no verification)"));

static llvm::cl::opt<std::string>
	clTransformBcFile("transform-output-bc", llvm::cl::init(""), llvm::cl::value_desc("file"),
			  llvm::cl::cat(clDebugging),
			  llvm::cl::desc("Output the transformed LLVM module as bitcode"));
static llvm::cl::opt<unsigned int>
...
void Config::checkConfigOptions() const
{
	...
	// Temporary for MIXER
	if (clMixer) {
		clDisableMMDetector = true;
		clModelType = ModelType::RA;
		clDisableEstimation = clDisableIPR = clDisableBAM = clDisableSymmetryReduction = true;
		clHelper = false;
		clBound = -1;
		clSchedulePolicy = SchedulePolicy::ltr;
	}
...
void Config::saveConfigOptions()
{
	...
	/* Always exclude runtime library functions from loop unrolling to prevent
	 * incomplete memory operations when --unroll bound is reached */
	noUnrollFuns.insert("memcpy");
	noUnrollFuns.insert("memmove");
	noUnrollFuns.insert("memset");
	noUnrollFuns.insert("memcmp");
	...
	programEntryFuns.insert(programEntryFuns.end(), clProgramEntryFunction.begin(),
				clProgramEntryFunction.end());
	if (programEntryFuns.empty())
		programEntryFuns.push_back("main");
	programEntryFun = programEntryFuns[0];
	...
	skipTransforms = clSkipTransforms;
	transformOnly = clTransformOnly;
	transformBcFile = clTransformBcFile;
	vLevel = clVLevel;
#ifdef ENABLE_GENMC_DEBUG
