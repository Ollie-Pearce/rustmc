TARGET_RUST_PROJECT=verify_test_benchmarks/pfds
MIXED_LANGUAGE=false
DEPDIR=$(pwd)

rm -rf verify_test_benchmarks/pfds
rm -rf test_results/*.txt
cd verify_test_benchmarks
git clone https://github.com/NeoCogi/pfds.git
cd ..
...
RUSTFLAGS="--emit=llvm-bc -Zpanic_abort_tests -C overflow-checks=off -C prefer-dynamic=no -C codegen-units=1 -C lto=no -C opt-level=0 -C debuginfo=2 -C llvm-args=--inline-threshold=9000 -C llvm-args=--bpf-expand-memcpy-in-order -C no-prepopulate-passes -C codegen-units=1 -C passes=ipsccp -C passes=globalopt -C passes=reassociate -C passes=argpromotion -C passes=typepromotion -C passes=lower-constant-intrinsics  -C passes=memcpyopt -Z mir-opt-level=0 " rustup run RustMC cargo test --target-dir target-ir --no-run
